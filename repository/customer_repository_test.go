...
func TestCustomerRepository(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "Save Success",
			mock: func() {
				customer := domain.Customer{CustomerID: 1, Name: "John Doe", Email: "john@example.com"}
				repo.EXPECT().Save(ctx, customer).Return(customer, nil)
			},
			method: func() (interface{}, error) {
				return repo.Save(ctx, domain.Customer{CustomerID: 1, Name: "John Doe", Email: "john@example.com"})
			},
			expect:    domain.Customer{CustomerID: 1, Name: "John Doe", Email: "john@example.com"},
			expectErr: false,
		},
		{
			name: "Save Failure",
